...
while (len(fill_indices[0])  > 0):
    ...
    while(len(edge_pixels[0]) > 0):
        ...
        random_pixel = random.randint(0, len(edge_pixels[0])-1)
        patch_center_i, patch_center_j = edge_pixels[0][random_pixel], edge_pixels[1][random_pixel]
        ...
        patch_mask = patch_to_fill > [0, 0, 0]
        #
        ...
        # pdb.set_trace()
        selected_center_i, selected_center_j = np.where(ssd_img == np.min(ssd_img))
        selected_center_i = selected_center_i[0] + patch_half_size
        selected_center_j = selected_center_j[0] + patch_half_size
        #
        ...
        iPatchTopLeft = patch_center_i - patch_half_size
        jPatchTopLeft = patch_center_j - patch_half_size
        fill_region[iPatchTopLeft:iPatchTopLeft+patchSize, jPatchTopLeft:jPatchTopLeft+patchSize] = 0
        fill_region_edge = utls.find_edge(fill_region)
        # update edge pixels
        edge_pixels = fill_region_edge.nonzero()
